...
{
  semantic_lidar_weight_sum_ = 0.0;
  semantic_lidar_weight_count_ = 0;
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    last_average_residual_ = effct_feat_num_ > 0 ? total_residual / effct_feat_num_ : -1.0;
    cout << "[ LIO ] Raw feature num: " << feats_undistort_->size() << ", downsampled feature num:" << feats_down_size_ 
         << " effective feature num: " << effct_feat_num_ << " average residual: " << last_average_residual_ << endl;
    ...
    for (int i = 0; i < effct_feat_num_; i++)
    {
      ...
      if (lidar_semantic_weight_evaluator_)
      {
        const double evaluated_weight = lidar_semantic_weight_evaluator_(point_body);
        if (evaluated_weight >= 0.0)
        {
          semantic_weight = std::max(0.05, std::min(1.0, evaluated_weight));
          semantic_lidar_weight_sum_ += semantic_weight;
          semantic_lidar_weight_count_++;
        }
      }
      R_inv(i) = lidar_factor_weight_ * semantic_weight /
                 (0.001 + sigma_l + ptpl_list_[i].normal_.transpose() * var * ptpl_list_[i].normal_);
