...
void VIOManager::updateStateInverse(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      for (int x = 0; x < patch_size; x++)
      {
        ...
        for (int y = 0; y < patch_size; ++y, img_ptr += scale)
        {
          ...
          if (!semantic_risk_visual_map_.empty() && semantic_risk_visual_map_.type() == CV_32FC1)
          {
            const int risk_u = u_ref_i + y * scale;
            const int risk_v = v_ref_i + x * scale;
            if (risk_u >= 0 && risk_u < semantic_risk_visual_map_.cols && risk_v >= 0 && risk_v < semantic_risk_visual_map_.rows)
            {
              const double semantic_weight = std::max(0.05, std::min(1.0, 1.0 - static_cast<double>(semantic_risk_visual_map_.at<float>(risk_v, risk_u))));
              semantic_visual_weight_sum_ += semantic_weight;
              semantic_visual_weight_count_++;
              const double sqrt_semantic_weight = std::sqrt(semantic_weight);
              H_sub.row(i * patch_size_total + x * patch_size + y) *= sqrt_semantic_weight;
              z(i * patch_size_total + x * patch_size + y) *= sqrt_semantic_weight;
            }
          }
          n_meas++;
        }
    ...
    if (error <= last_error)
    {
      ...
      const double sqrt_visual_weight = std::sqrt(visual_factor_weight_);
      H_sub *= sqrt_visual_weight;
      z *= sqrt_visual_weight;
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  if (!semantic_risk_visual_map_.empty() &&
      (semantic_risk_visual_map_.cols != img.cols || semantic_risk_visual_map_.rows != img.rows))
  {
    cv::resize(semantic_risk_visual_map_, semantic_risk_visual_map_, img.size(), 0, 0, cv::INTER_LINEAR);
  }
  img_rgb = img.clone();
  img_cp = img.clone();
  semantic_visual_weight_sum_ = 0.0;
  semantic_visual_weight_count_ = 0;
  // img_test = img.clone();
