...
public:
  ...
  double last_average_residual_ = -1.0;
  double lidar_factor_weight_ = 1.0;
  std::function<double(const V3D &)> lidar_semantic_weight_evaluator_;
  double semantic_lidar_weight_sum_ = 0.0;
  size_t semantic_lidar_weight_count_ = 0;
  int scan_count = 0;
  StatesGroup state_;
  V3D position_last_;
  ...
  void setLidarFactorWeight(double weight) { lidar_factor_weight_ = std::max(0.05, std::min(1.0, weight)); }
  void setLidarSemanticWeightEvaluator(const std::function<double(const V3D &)> &evaluator) { lidar_semantic_weight_evaluator_ = evaluator; }
