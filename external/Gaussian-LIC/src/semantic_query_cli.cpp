...
int main(int argc, char** argv)
{
    ...
    if (bundle_dir.empty() || query_npy.empty())
    {
        std::cerr << "Usage: semantic_query_cli --bundle-dir <dir> --query-npy <file> [--topk N] [--ignore-mask] [--out-json file]" << std::endl;
        return 2;
    }
    ...
    {
        std::cerr << "[semantic_query_cli] " << e.what() << std::endl;
        return 1;
    }
}
