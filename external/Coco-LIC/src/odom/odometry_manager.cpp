...
namespace cocolic
{
                                          ...
                                          const std::string &bag_path)
  {
    ...
    boost::filesystem::path cache_dir = boost::filesystem::path(cache_path_).parent_path();
    boost::system::error_code error;
    boost::filesystem::create_directories(cache_dir, error);
    if (error)
    {
      std::cerr << "Failed to create trajectory output directory "
                << cache_dir.string() << ": " << error.message() << std::endl;
      return false;
    }
    return true;
  }
